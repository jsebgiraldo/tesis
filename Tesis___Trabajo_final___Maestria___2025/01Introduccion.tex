...
\hline
\textbf{Zigbee Pro (2.4 GHz)} & Alcance limitado + latencia multi-hop & 50-100 m @ 2.4 GHz requiere topología mesh multi-hop (latencia +15-30 ms/hop × 3-5 hops = +45-150 ms). Interferencia WiFi co-channel. & HaLow: 1 km alcance directo, topología star/tree \textbf{sin multi-hop}. Sub-GHz 900 MHz (mejor penetración muros, -3 dB vs -10 dB 2.4 GHz). \tabularnewline
\hline
\textbf{LoRaWAN (privado)} & Throughput extremadamente bajo & 50 kbps máximo (SF7 @ 125 kHz), 5.5 kbps típico (SF10). \textbf{Insuficiente para waveforms} (400 kbps). Latencia 1-5s por ALOHA sin CSMA. & HaLow: 250 kbps mínimo (MCS0), \textbf{45× superior} LoRa típico. CSMA/CA garantiza latencia <100 ms. \tabularnewline
\hline
\end{tabular}}
\end{table}
...
\textbf{NB-IoT - Requisito NO NEGOCIABLE de throughput:} La transmisión de waveforms de calidad de energía (power quality) según IEC 61000-4-30 requiere muestreo mínimo 10 kHz (10,000 samples/segundo) para capturar armónicos hasta 5to orden (50 Hz × 5 = 250 Hz). Con resolución 12-bit/sample (1.5 bytes) y overhead CoAP/UDP/IP comprimido 6LoWPAN (factor 1.3×), el throughput mínimo requerido es:

\[
\text{Throughput}_{min} = 10,000 \frac{samples}{s} \times 1.5 \frac{bytes}{sample} \times 1.3 = 19,500 \frac{bytes}{s} = 156 \text{ kbps}
\]

Para transmisión \textbf{simultánea de 3 fases} (trifásico) + metadata timestamp: $156 \times 3 = 468$ kbps requeridos. NB-IoT con 250 kbps nominal (Release 13) presenta un \textbf{déficit del 62\%} que no es mitigable mediante compresión sin introducir distorsión de armónicos (error >5\% THD inadmisible para análisis power quality). Wi-Fi HaLow con 43.3 Mbps @ 8 MHz BW (MCS9-10) supera el requisito por factor \textbf{92×}, permitiendo además transmisión de múltiples medidores simultáneamente sin colisiones.
...
HaLow con \textbf{CAPEX único} \$40-60/radio (Alfa Network AWUS036ACS) y \$0 OPEX presenta reducción \textbf{90\% TCO 10 años}: \$107/medidor (arquitectura propuesta, 5 años) vs \$1,800-3,000/medidor 5G (\$180-300/año × 10 años).
