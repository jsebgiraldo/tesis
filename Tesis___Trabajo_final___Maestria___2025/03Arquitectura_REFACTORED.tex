...
El primer nivel comprende los medidores inteligentes (Itron SL7000, Landis+Gyr E650) con interfaces DLMS/COSEM sobre RS-485, y los nodos adaptadores IoT basados en ESP32-C6 con radio IEEE 802.15.4 para protocolo Thread. Cada medidor se conecta mediante puerto óptico o RS-485 (9600 bps) a un nodo IoT que ejecuta firmware ESP-IDF 5.1 con stack OpenThread 1.3.0. Los nodos operan en modo \textit{Minimal End Device} (MED, RX always-on) alimentados directamente desde el puerto auxiliar 5V del medidor, consumiendo 135 mW promedio (27 mA) en operación continua 24/7~\cite{krawiecEnergyFootprintReliability2025}. La traducción de protocolos DLMS/COSEM a CoAP/JSON se realiza en el nodo IoT, encapsulando códigos OBIS (voltage, current, active/reactive energy) en payloads de 200 bytes que se transmiten mediante Thread mesh hacia el siguiente nivel.
...
\hline
\textbf{Consumo energético} & 19 mA activo (MED) & 3-5 mA sleep & Thread mayor consumo como MED (RX always-on). Nodos alimentados desde medidor (5V disponible), no limitante. \\
\hline
