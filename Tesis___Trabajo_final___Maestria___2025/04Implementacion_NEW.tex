...
\begin{itemize}
    ...
    \item \textbf{Interfaces físicas:} UART $\times$ 3 (RS-485 adaptador MAX485), GPIO $\times$ 22
...
\begin{itemize}
    \item \textbf{Telemetría:} Persistencia PostgreSQL con retención 7 días (capacidad 128 GB SD card suficiente para 2.88M msgs/mes $\times$ 7 días = 672K msgs buffered)
...
\begin{itemize}
    \item \textbf{eDRX Cycle:} 10.24 s (máximo Cat-M1). Gateway monitorea paging 1$\times$/10s, reduce consumo RX 99\% (60 mA $\rightarrow$ 0.58 mA)
    ...
    \item \textbf{Patrón operación:} TX telemetry 1$\times$/min $\rightarrow$ 30 s active $\rightarrow$ 29.5 min PSM (3 $\mu$A) $\rightarrow$ repeat
...
\begin{itemize}
    ...
    \item \textbf{Splitters Ópticos Pasivos:} 3$\times$ splitters 1:8 instalados en postes intermedios (reducción cableado punto-a-punto 75\%)
    \begin{itemize}
    ...
    \item \textbf{ONTs Gateway:} 3$\times$ Huawei HG8245H instalados en ubicaciones gateways Raspberry Pi
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Budget óptico total: OLT TX +4 dBm - (2.4 km $\times$ 0.35 dB + 3$\times$ splitters $\times$ 10.5 dB + conectores 2 dB) = +4 - 33.3 = -29.3 dBm llegada ONT (vs -27 dBm sensibilidad = 2.3 dB margen, suficiente)
...
\textbf{Tiempo instalación:} 12 min promedio/nodo (técnico experimentado). 30 nodos = 6h labor (2 técnicos $\times$ 3h).
