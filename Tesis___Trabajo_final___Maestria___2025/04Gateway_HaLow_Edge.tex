...
El Nivel 3 de la arquitectura implementa inteligencia distribuida mediante gateways edge que ejecutan procesamiento local, almacenamiento persistente y sincronización bidireccional con servidor on-premise. La implementación desplegó \textbf{3 routers Alfa Networks Tube AHM} (chipset MT7628 + radio Morse Micro MM6108) formando red mesh IEEE 802.11s HaLow en ubicaciones estratégicas de alta cota, proporcionando cobertura de 0.8 km² en zona urbana residencial montañosa (desnivel 180 m entre puntos extremos). \textbf{2 gateways Raspberry Pi 4} se conectan como \textbf{clientes STA inalámbricos exclusivamente a los 2 Mesh Points} (Tube-02 y Tube-03) mediante módulos USB Alfa AHPE-HM (también basados en MM6108), eliminando necesidad de cableado Ethernet punto-a-punto. El Mesh Gate (Tube-01) provee únicamente backhaul Ethernet hacia switch local (server on-premise conectado al mismo switch) sin gateways edge co-ubicados. Este capítulo enfoca en arquitectura HaLow mesh 802.11s y gateways edge RPi4; la infraestructura de servidor on-premise y conectividad WAN se documentan en Capítulo~\ref{chap:resultados}.

Cada Gateway Raspberry Pi 4 desempeña tres roles arquitectónicos críticos: (1) \textbf{Thread Border Router} mediante nRF52840 USB Dongle con stack OpenThread, interconectando red mesh Thread local (IEEE 802.15.4, 2.4 GHz) con infraestructura IP tradicional vía servicios NAT64/DNS64, (2) \textbf{Plataforma Edge Computing} ejecutando ThingsBoard Edge 3.6.2 en stack Docker con 6 microservicios para Rule Engine local, agregación temporal TimescaleDB, y buffer offline 72 horas de telemetría, y (3) \textbf{Cliente HaLow STA} conectándose inalámbricamente a red mesh Alfa Networks Tube AHM operando en banda 902-928 MHz sub-1 GHz con alcance 280-520 m hacia múltiples Border Routers Thread distribuidos geográficamente.

\textbf{Clarificación arquitectónica crítica:} Los routers Alfa Networks Tube AHM son dispositivos \textit{comerciales standalone} con procesador integrado MT7628 (MIPS @ 580 MHz) ejecutando OpenWRT 23.05 y radio HaLow MM6108. \textbf{NO son módulos conectados directamente a Raspberry Pi 4}; son routers independientes que forman la infraestructura mesh HaLow. Los 2 gateways Raspberry Pi 4 actúan como clientes finales (STAs) conectándose \textbf{exclusivamente a los 2 Mesh Points} (Tube-02 y Tube-03) mediante adaptadores USB HaLow externos, \textbf{NO al Mesh Gate} (Tube-01). El Mesh Gate se dedica únicamente a proveer backhaul Ethernet hacia switch local (server on-premise con ThingsBoard conectado al mismo switch). Esta arquitectura distribuida permite cobertura extendida (520 m entre puntos extremos) sin requerir cableado estructurado Ethernet, reduciendo CAPEX deployment 60\% vs alternativas cableadas.
...
\begin{itemize}
    ...
    \item \textbf{Conectividad Red:}
        \begin{itemize}
        \item \textbf{HaLow 802.11ah (conexión primaria):} Módulo USB Alfa AHPE-HM (Morse Micro MM6108), modo STA, banda 902-928 MHz @ 4 MHz BW, throughput 4 Mbps, alcance 280-520m hacia routers Alfa Tube AHM mesh. \textit{Esta es la conexión principal de backhaul para telemetría AMI.}
        \item Ethernet Gigabit (Broadcom GENET, throughput real 940 Mbps), disponible pero NO usado en deployment piloto (arquitectura 100\% wireless)
...
\begin{itemize}
    \item \textbf{Tube-01 (Mesh Gate):} Instalado en ubicación central estratégica, conexión Ethernet GbE hacia switch local (server on-premise ThingsBoard conectado al mismo switch, arquitectura detallada Capítulo~\ref{chap:resultados}), coordenadas GPS 6.2476°N, -75.5658°W, altura 1650 msnm (azotea edificio). \textbf{Sin gateways Raspberry Pi 4 conectados} (Mesh Gate dedicado exclusivamente a routing mesh y backhaul Ethernet). Role: Root mesh gate, propaga default route hacia red local via HWMP, responde PREQ broadcasts con costo métrico 0.
    
    \item \textbf{Tube-02 (Mesh Point relay):} Instalado poste eléctrico intermedio, distancia 280 m desde Tube-01, coordenadas 6.2494°N, -75.5672°W, altura 1540 msnm. \textbf{Gateway RPi4-01 conectado como cliente STA HaLow} a este Mesh Point, gestionando 15 nodos Thread locales. Role: Relay tráfico entre Gateway RPi4-01 y Mesh Gate upstream, forwarding bidireccional.
    
    \item \textbf{Tube-03 (Mesh Point relay):} Instalado poste extremo cobertura, distancia 520 m desde Tube-01 / 380 m desde Tube-02, coordenadas 6.2512°N, -75.5695°W, altura 1470 msnm (desnivel -180 m vs Tube-01). \textbf{Gateway RPi4-02 conectado como cliente STA HaLow} a este Mesh Point, gestionando 15 nodos Thread locales. Role: Relay tráfico entre Gateway RPi4-02 y Mesh Gate, path primario via Tube-02 (1 hop), path secundario directo Tube-01 (1 hop, usado si Tube-02 falla).
\end{itemize}
