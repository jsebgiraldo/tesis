...
El capítulo se estructura en cinco secciones principales: §\ref{sec:gateway-arquitectura} presenta la arquitectura hardware Raspberry Pi 4 y el stack de software Docker con 6 microservicios containerizados, §\ref{sec:gateway-otbr} documenta la implementación del OpenThread Border Router con servicios NAT64/DNS64 para interconexión Thread-IPv6, §\ref{sec:gateway-halow} detalla la red HaLow mesh con routers MikroTik Tube AHM y protocolo HWMP, §\ref{sec:gateway-tb-edge} describe el despliegue de ThingsBoard Edge con Rule Chains para procesamiento local y reducción 72\% del tráfico WAN, y §\ref{sec:gateway-conclusiones} sintetiza los logros y transiciones al Capítulo 5.
