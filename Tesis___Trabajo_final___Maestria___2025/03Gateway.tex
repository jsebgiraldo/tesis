...
El primer nivel está compuesto por dispositivos sensores y actuadores, alimentados directamente desde la infraestructura del medidor (5V). Estos nodos implementan las funciones mínimas necesarias para:
...
\begin{itemize}
    ...
    \begin{itemize}
        \item 2$\times$ USB 3.0 (5 Gbps)
        \item 2$\times$ USB 2.0 (480 Mbps)
        \item 40-pin GPIO header (alimentación 3.3V/5V, I2C, SPI, UART)
        \item 1$\times$ PCIe 1$\times$ lane via GPIO 40-pin (requiere HAT adapter)
    \end{itemize}
    \item \textbf{Alimentación}: 5V/3A via USB-C (fuente oficial Raspberry Pi)
    \item \textbf{Dimensiones}: 85 mm $\times$ 56 mm $\times$ 17 mm (sin HATs)
...
PCIe nativo & Sí (1$\times$ lane via HAT) & Sí (miniPCIe) \\
...
\begin{itemize}
    ...
    \item \textbf{Throughput}: Hasta 40 Mbps (MCS10, 8 MHz, MIMO 1$\times$1)
...
\begin{itemize}
    ...
    \item \textbf{Interfaz}: PCIe 1$\times$ Gen 2 (5 Gbps, 500 MB/s teórico)
...
\begin{itemize}
    ...
    \item \textbf{Antenas}: 2x conectores U.FL para diversidad MIMO 2$\times$2.
...
\begin{itemize}
    \item \textbf{Compresión}: 10-20$\times$ reducción en espacio disco mediante algoritmos adaptativos
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Buffer local debe almacenar >28k mensajes (300 medidores $\times$ 96 lecturas/día).
...
\begin{enumerate}
    ...
    \item Genera respuesta: "Anomalía detectada: consumo 10$\times$ superior al promedio. Posible bypass del medidor o falla de CT (transformador de corriente). Recomendar inspección física."
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Topología estrella (AP único)}: 10 DCUs $\times$ 250 nodos Thread = 2,500 endpoints, cobertura 3 km, latencia <50 ms.
        \item \textbf{Topología mesh (3 gateways 802.11s)}: 30 DCUs $\times$ 250 nodos = 7,500 endpoints, cobertura 9 km, latencia <200 ms, resiliencia ante fallo de gateway.
        \item \textbf{Topología EasyMesh (1 Controller + 4 Agents)}: 50 DCUs $\times$ 250 nodos = 12,500 endpoints, roaming transparente, carga balanceada automática.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{OPEX}: Backhaul HaLow sin costo recurrente vs. 9 planes LTE $\times$ \$30/mes = ahorro \$3,240/año.
    ...
    \begin{itemize}
        ...
        \item \textbf{PostgreSQL + TimescaleDB}: Series temporales optimizadas, compresión 10-20$\times$, particionamiento automático, retención 90 días, queries agregadas (time\_bucket), hypertables con >3000 IOPS en SSD NVMe.
    ...
    \item \textbf{Escalabilidad}: Soporta hasta 10 DCUs simultáneos vía HaLow AP (cada DCU con 250 nodos Thread) con agregación en TB Edge antes de enviar al cloud. Mesh y EasyMesh multiplican capacidad 3-5$\times$ sin rediseño arquitectónico.
...
\hline
802.11s Mesh & Área extendida sin infraestructura & Auto-healing, 3$\times$ cobertura, CAPEX reducido & 3 gateways mesh en zona rural (9 km, 900 medidores) \\
\hline
