...
\begin{itemize}
\item \textbf{Reducción de latencia >60\%:} La arquitectura propuesta alcanzó \textbf{latencia extremo-a-extremo (\textit{end-to-end}, E2E)} medida de 672±34~ms (n=259,200 mensajes, 90 días), con presupuesto teórico (\textit{latency budget}) de 248~ms. Comparada con arquitectura centrada en la nube (\textit{cloud-centric}) línea base (\textit{baseline}) HTTP/REST que registró 3247±118~ms, la reducción es de \textbf{79.3\%} (p<0.0001). La \textbf{latencia de procesamiento local en el borde (\textit{edge processing})} fue medida experimentalmente alcanzando 8±2~ms promedio (P99=12~ms), habilitando decisiones en tiempo real que cumplen requisitos IEC 62056 (<1~s) con margen de 33\%.\footnote{Latencia E2E medida 672~ms vs presupuesto teórico 248~ms --- discrepancia atribuida a variabilidad LTE en hora pico y contención MAC HaLow con 60 nodos simultáneos. El presupuesto teórico se calculó mediante suma de componentes individuales medidos (Cap.~\ref{chap:gateway-halow} §4.13.1).}
...
\item \textbf{Eficiencia energética:} Nodos IoT alimentados directamente desde el puerto auxiliar 5V del medidor operan continuamente como MED (RX always-on, 27 mA promedio), eliminando necesidad de gestión de energía o baterías. Thread soporta adicionalmente roles SED (\textit{Sleepy End Device}) para otros escenarios IoT con dispositivos autónomos. HaLow con TWT habilita eficiencia en nodos intermedios (1 muestra/min, 0.2\% ciclo de trabajo).
...
\begin{itemize}
\item \textbf{Telemetría normal:} 10,000 medidores $\times$ 180 bytes / 60 s = 30 KB/s = 240 kbps
\item \textbf{Waveforms:} 10,000 $\times$ 2.5 KB / 900 s = 27.8 KB/s = 222 kbps
\item \textbf{Heartbeat:} 10,000 $\times$ 40 bytes / 300 s = 1.33 KB/s = 10.6 kbps
\item \textbf{Overhead protocolar:} Thread (15\%) + HaLow (8\%) + MQTT (12\%) = 35\% adicional
\item \textbf{Total throughput downlink (medidores → gateway):} (240 + 222 + 10.6) $\times$ 1.35 = \textbf{638 kbps}
...
\begin{itemize}
\item \textbf{Tráfico cloud sync:} 638 kbps $\times$ 0.28 = 179 kbps (\textasciitilde{}1.3 GB/mes)
...
\item \textbf{Capacidad estimada OTBR single-instance:} 8 Routers $\times$ 32 End Devices = \textbf{256 dispositivos Thread máximo}
\end{itemize}
...
\begin{itemize}
\item $\lambda$ (tasa llegada paquetes): 10,000 medidores $\times$ 1 pkt/60s = 167 pkt/s
...
\item \textbf{Throughput efectivo:} 650 $\times$ 0.68 = \textbf{442 kbps por AP}
\end{itemize}
...
\begin{itemize}
\item \textbf{CPU:} 34\% $\times$ (10,000 / 10) = 3,400\% = \textbf{34 cores necesarios} (8.5$\times$ Raspberry Pi 4)
\item \textbf{RAM:} 3,735 MB $\times$ 1,000 = \textbf{3.6 TB RAM} (escalado no es lineal, ver análisis siguiente)
\item \textbf{Storage (telemetría 1 año):} 180 bytes/msg $\times$ 10K $\times$ (60s/24h/365d) = \textbf{947 GB/año}
\end{itemize}
...
\item \textbf{TimescaleDB:} Continuous aggregates + compression (política 7 días) + partitioning reduce footprint RAM a ~50 MB + (10 KB $\times$ número dispositivos activos últimas 24h). Para 10K: $50 + (10 \times 10,000/1024) =$ \textbf{148 MB RAM} (conexiones activas)
...
\item \textbf{Throughput uplink:} 40 $\times$ 16 kbps = 640 kbps (dentro capacidad 442 kbps $\times$ 2 APs)
...
\begin{itemize}
\item \textbf{Hardware:} Dell PowerEdge R750 (2$\times$ Xeon Silver 4314 16-core, 768 GB RAM, RAID SSD 4 TB)
...
\hline
Conectividad LTE (2 SIM $\times$ \$30/mes) & 720 \\
\hline
Energía eléctrica (41 dispositivos $\times$ 15W $\times$ \$0.12/kWh) & 648 \\
\hline
...
\hline
Personal soporte técnico (0.5 FTE $\times$ \$45K) & 22,500 \\
\hline
...
\item \textbf{OPEX (5 años):} \$43,000 $\times$ 5 = \$215,000
...
\begin{itemize}
\item \textbf{CAPEX:} Medidor + modem LTE = \$55 $\times$ 10,000 = \$550,000
\item \textbf{OPEX:} Plan datos \$8/mes $\times$ 10,000 = \$80,000/mes = \$960,000/año
\item \textbf{TCO 5 años:} \$550K + (\$960K $\times$ 5) = \textbf{\$5,350,000}
\end{itemize}
...
\item Partitioning por mes (mejora queries temporales 10$\times$)
...
\begin{itemize}
    ...
    \item \textbf{Total: 16.5 PA} → equipo sostenido 3 investigadores $\times$ 5.5 años
\end{itemize}
...
\textbf{Recursos:} 0.5 PA investigador junior + \$1.5K hardware (3$\times$ GW16167 @ \$150 + antennas + instrumentación).
...
\textbf{Recursos:} 0.5 PA ingeniero de campo + \$3.5K (10$\times$ módulos GW16167 @ \$150 + labor on-site 10$\times$ \$25 + contingencia).
