...
\begin{table}[H]
\centering
\caption{Mapeo arquitectura propuesta a estándar ISO/IEC 30141:2024}
\label{tab:iso30141-mapping}
\small
\begin{tabular}{|l|l|l|}
\hline
\rowcolor{blue!20}
\textbf{Functional Entity} & \textbf{Componente} & \textbf{Implementación} \\
\hline
\textbf{Sensing FE} & Medidores + Thread & IEC 62056-21, ESP32-C6, 802.15.4 \\
\hline
\textbf{Communication FE} & Multi-radio gateway & Thread mesh 250 nodos, HaLow 1 km \\
\hline
\textbf{Processing FE Edge} & RPi 4 & TB Edge, Python, 4 GB RAM \\
\hline
\textbf{Processing FE Cloud} & TB Cloud cluster & Microservicios, Kafka, K8s \\
\hline
\textbf{Storage FE Local} & Gateway TimescaleDB & 7 días retention, 256 GB SSD \\
\hline
\textbf{Storage FE Cloud} & TB PostgreSQL & 5 años, compression, S3 archival \\
\hline
\textbf{Security FE} & Thread + HaLow + TLS & AES-128-CCM, WPA3-SAE, mTLS 1.3 \\
\hline
\textbf{Management FE} & TB Device Mgmt & OTA updates, UCI OpenWRT, Grafana \\
\hline
\textbf{Application FE} & REST + WebSocket & TB REST API, CoAP↔HTTP adapter \\
\hline
\rowcolor{yellow!20}
\textbf{Conformidad} & \textcolor{green}{\textbf{Completa}} & 7/7 FEs implementadas \\
\hline
\end{tabular}
}
\end{table}
...
\hline
\rowcolor{gray!20}
\textbf{Limitación} & \textbf{Impacto} & \textbf{Mitigación} \\
\hline
IEEE 2030.5 polling & 288 MB/día tráfico en 10K disp & CoAP Observe, -90\% tráfico \\
\hline
Escalabilidad 50K & Cuello botella, latencia >5s & Edge distribuido, 10 GW×10K \\
\hline
Dependencia WAN & Incomunicación en fallas & Buffer local 7d, autónomo \\
\hline
ISO 30141 abstracción & Falta interoperabilidad & Especificación CoAP/LwM2M \\
\hline
Mapeo 4 vistas & Documentación incompleta & Tabla FE→componentes \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Consumo adicional 12\%:} Thread RX corriente 6.5 mA vs Zigbee 5.8 mA (medido en nRF52840 vs CC2652R, RSSI -60 dBm, canal 15). En esta arquitectura, los nodos operan como MED (RX always-on) alimentados desde el puerto auxiliar 5V del medidor, por lo que la diferencia de consumo no es limitante. Para escenarios con dispositivos autónomos, Thread soporta modo SED con duty-cycle optimizado.
...
\begin{table}[H]
\centering
\caption{Comparación LwM2M vs protocolos alternativos gestión dispositivos}
\label{tab:lwm2m-comparison}
\small
\begin{tabular}{|l|l|l|l|l|}
\hline
\rowcolor{blue!20}
\textbf{Característica} & \textbf{LwM2M 1.2} & \textbf{MQTT} & \textbf{HTTP REST} & \textbf{TR-069} \\
\hline
\textbf{Overhead} & \textcolor{green}{20-40 B} & 100-300 B & 200-500 B & 500-1500 B \\
\hline
\textbf{Gestión} & \textcolor{green}{Nativa} & Custom & Custom & CPE WAN \\
\hline
\textbf{OTA} & \textcolor{green}{Std Obj 5} & Custom & Custom & Download \\
\hline
\textbf{Observe} & \textcolor{green}{Nativo} & MQTT native & Polling/SSE & Notification \\
\hline
\textbf{Seguridad} & \textcolor{green}{DTLS-PSK} & TLS & TLS & SOAP/TLS \\
\hline
\textbf{Interop} & \textcolor{green}{Multi-vendor} & Propietario & Propietario & Broadband \\
\hline
\textbf{Eficiencia} & \textcolor{green}{Excelente} & Buena & Regular & Pobre \\
\hline
\textbf{Aplicabilidad} & \textcolor{green}{Alta} & Media & Baja & Nula \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
% TODO: Imagen wisun-protocol-stack.jpg corrupta (0.5 KB). Reemplazar con diagrama real.
\fbox{\parbox{0.8\textwidth}{\centering\textbf{[PLACEHOLDER]}\\Arquitectura protocolar Wi-SUN FAN\\TODO: Insertar diagrama de stack Wi-SUN}}
\caption{Arquitectura protocolar Wi-SUN FAN (Field Area Network) para Smart Grid comparada implícitamente con Thread. Wi-SUN implementa stack completo IEEE 802.15.4g/e (PHY sub-GHz 868/915 MHz, alcance 1-3 km vs Thread 2.4 GHz 30-50 m) con 6LoWPAN compression (idéntico a Thread), IPv6 routing mediante RPL (Routing Protocol for Low-Power, RFC 6550) con construcción DODAG tree-based, y seguridad IEEE 802.1X (EAP-TLS certificates). \textbf{Diferencias arquitectónicas clave vs Thread (ver Sección~\ref{sec:nodo-thread} Capítulo~\ref{chap:nodo-iot}):} (1) \textit{Frecuencia:} Wi-SUN sub-GHz (mejor penetración +8-12 dB paredes concreto, mayor alcance 2-5 km) vs Thread 2.4 GHz (mayor throughput 250 kbps, menor latencia <50 ms, chipsets commoditized), (2) \textit{Routing:} Wi-SUN RPL tree-based con upward/downward tables separadas (+4-8 KB RAM overhead) vs Thread MLE mesh peer-to-peer con single unified table convergencia <5s, (3) \textit{Commissioning:} Wi-SUN requiere certificados PKI X.509 (complejidad operacional alta, infrastructure CA/RADIUS) vs Thread passphrase simple (PAKE protocol ECC P-256), (4) \textit{Ecosistema:} Wi-SUN utilities/industrial (millones smart meters deployed Japón/Europa) vs Thread domótico (Google/Apple/Amazon Matter stack, 300+ Thread Group members). Esta tesis seleccionó Thread por: menor latencia (<50 ms vs 200-500 ms Wi-SUN) crítica para comandos Demand Response, throughput superior suficiente para medidores trifásicos (451 bytes payload cada 15 min), commissioning simplificado sin PKI infrastructure, y costo hardware inferior (ESP32-C6 \$5 vs Wi-SUN modules CC1312R \$2.70 chip pero +\$15 certificación FAN)~\cite{tiCC1312RWiSUN2024}. Wi-SUN ventajoso para deployments outdoor rurales >1 km donde alcance sub-GHz crítico supera trade-offs latencia/complejidad.}
\label{fig:wisun-protocol-stack}
\end{figure}
...
El Nivel 2 interconecta múltiples redes de campo (Nivel 1) hacia gateways de procesamiento edge (Nivel 3), requiriendo tecnologías con alcance >1 km y throughput >1 Mbps para agregar cientos de medidores. Esta sección revisa primero las tecnologías LPWAN alternativas para contexto comparativo, luego detalla las dos tecnologías implementadas en la arquitectura híbrida.

% ============================================================================
% SECCIÓN §2.4.1: TECNOLOGÍAS LPWAN (LoRaWAN, NB-IoT, Wi-Fi HaLow)
% Integrada desde: 02MarcoTeorico_SECCION_2_4_1_NUEVA.tex
...
\textbf{Ventajas HaLow vs Alternativas Sub-GHz:} Comparado con LoRaWAN (50 kbps máx, latencia >1s) o Wi-SUN (300 kbps máx), HaLow provee 10-100$\times$ mayor throughput manteniendo alcance similar~\cite{khorovSurveyIEEE802112016}. Comparado con LTE-M/NB-IoT, HaLow elimina OPEX mensual carrier (\$8-12/mes/dispositivo) y opera en banda ISM libre~\cite{adameTimeVariantChannelModeling2019}.

\textbf{Aplicabilidad AMI:} Un Access Point HaLow puede servir 100-500 medidores en radio 1 km (densidad urbana típica 200-300 medidores/km²), agregando 4 Mbps $\times$ 0.6 duty cycle = 2.4 Mbps efectivo. Asumiendo 2.5 KB/lectura $\times$ 96 lecturas/día/medidor = 240 KB/día/medidor, 500 medidores generan 120 MB/día = 11 kbps promedio, bien dentro de capacidad HaLow~\cite{alsafranChallengesImplementingIoT2025}.
...
\textbf{Relevancia para Smart Metering:} Este caso valida que para aplicaciones de telemetría IoT (smart metering con throughput 1-10 kbps por dispositivo), las distancias operacionales de 2-3 km requeridas en esta tesis son \textit{altamente conservadoras} dado que HaLow demostró capacidad de soportar aplicaciones de video (>1 Mbps) a 7.5 km. El éxito comercial en agricultura con requisitos de throughput 100$\times$ superiores confirma la viabilidad técnica y económica de HaLow como tecnología de backhaul para Advanced Metering Infrastructure en entornos semi-urbanos con concentradores DCU separados 2-3 km del gateway central.
...
\begin{itemize}
    ...
    \item \textbf{Interfaces I/O:} 5$\times$ Gigabit Ethernet, RS485/RS232/RS422 serial, 4$\times$ DI/DO digitales, 4$\times$ AI/AO analógicos, USB 2.0
...
\begin{itemize}
    ...
    \item \textbf{Comparación económica:} RAH305 (\$600-800) vs gateway propuesto (\$295 BOM Cap 4) = \textbf{RAH305 es 2.0-2.7$\times$ más costoso}. Sobrecosto justificado en aplicaciones industriales genéricas (SCADA, surveillance) pero innecesario para AMI donde Thread+HaLow+Edge-AI son suficientes.
\end{itemize}

\textbf{Conclusión RAH305:} Valida viabilidad comercial del chipset MM6108 y arquitectura multi-radio, pero evidencia sobreespecificación (Wi-Fi 6 + Bluetooth + GNSS no requeridos en AMI fijo) y costo 2-3$\times$ superior para aplicación Smart Energy específica.
...
\begin{itemize}
    ...
    \item \textbf{Potencia TX:} Hasta +26 dBm (vs +23 dBm MM6108) = \textbf{+3 dB mejora = 2$\times$ potencia radiada}
...
\hline
Potencia TX & +23 dBm & \textcolor{blue}{+26 dBm (+3 dB)} & 2$\times$ potencia radiada, +40\% alcance \\
\hline
