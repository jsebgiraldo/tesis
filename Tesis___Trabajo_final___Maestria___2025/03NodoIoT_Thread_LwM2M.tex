...
§\ref{sec:nodo-hardware} presenta la plataforma de referencia: SoC~ESP32-C6 con radio IEEE~802.15.4 y transceptor RS-485 con aislamiento galvánico. §\ref{sec:nodo-medidor-dlms} especifica el medidor Emsitech~P2000-T, el protocolo DLMS/COSEM y los 27~códigos OBIS implementados. §\ref{sec:nodo-lwm2m} define el Objeto~10242 (\emph{3-Phase Power Meter}) con sus 27~recursos y el perfil LwM2M configurado en ThingsBoard. §\ref{sec:nodo-firmware} detalla el \emph{firmware} Zephyr~RTOS con pila CoAP/OpenThread, ciclo principal y parámetros Kconfig. §\ref{sec:nodo-pila-protocolos} describe la red de campo Thread: capa IEEE~802.15.4, topología, auto-formación y direccionamiento IPv6, \textbf{agotando el tema Thread aquí} según la Regla de Exclusión. §\ref{sec:nodo-pruebas} presenta el análisis de carga y las pruebas de latencia que validan H3 ($<$250\,ms).
...
\begin{table}[H]
\centering
\caption{Componentes principales de la plataforma de referencia del nodo IoT}
\label{tab:plataforma-referencia}
\begin{tabular}{llp{6.5cm}}
\toprule
\textbf{Componente} & \textbf{Parámetro} & \textbf{Valor} \\
\midrule
\multirow{5}{*}{\parbox{3cm}{\textbf{SoC / Módulo}\\\small XIAO ESP32-C6~\cite{seeedstudioXIAOESP32C62024}}}
  & CPU          & RISC-V RV32IMAC @ 160\,MHz \\
  & SRAM / Flash & 512\,KB / 4\,MB \\
  & Radio        & IEEE~802.15.4: TX +21\,dBm, RX $-$103\,dBm \\
  & Dimensiones  & 21$\times$17{,}5\,mm \\
  & Precio       & \$3{,}20\,USD (volumen) \\
\midrule
\multirow{3}{*}{\parbox{3cm}{\textbf{Transceptor RS-485}\\\small MAX485~\cite{seeedstudioRS485Breakout2024}}}
  & Aislamiento  & Galvánico 2{,}5\,kV \\
  & Interfaz     & A$+$, B$-$, GND; terminación 120\,$\Omega$ \\
  & Alimentación & 5\,V desde puerto auxiliar del medidor \\
\bottomrule
\end{tabular}
\end{table}
...
Credenciales Thread estáticas en \texttt{prj.conf} (canal, Network Key, PAN~ID). Configuraciones clave del proyecto \texttt{ami-lwm2m-client~v0.4.0}:
...
# AMI: single-phase mode (pre-skips Phase S/T OBIS codes)
CONFIG_AMI_SINGLE_PHASE=y
\end{verbatim}

Binario: 634\,KB de 4\,MB Flash (15\%), 252\,KB de 437\,KB SRAM (57\%). Modo \texttt{CONFIG\_AMI\_SINGLE\_PHASE=y} activo por defecto: omite fases S y T (12~códigos OBIS), reduciendo a 15~lecturas por ciclo y ahorrando $\approx$5\,s de sondeo en medidores monofásicos.
...
Los nodos del piloto se configuran como \textbf{FTD} (\emph{Full Thread Device}, \texttt{CONFIG\_OPENTHREAD\_FTD=y}), lo que permite que cualquier nodo pueda promover a Router y reforzar la malla. En la validación experimental el nodo opera como \emph{Child} del Border Router; con RX siempre activo garantiza entrega $<$100\,ms para comandos Demand Response. Alimentados desde el medidor (5\,V, 200\,mA disponibles), el consumo en modo FTD no representa restricción.
