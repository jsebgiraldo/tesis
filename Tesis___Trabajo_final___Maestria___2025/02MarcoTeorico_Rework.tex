...
\textbf{ThingsBoard Edge:} Replica funcionalidad completa de plataforma IoT en gateways locales, sincronizando entidades (devices, dashboards, rule chains) bidireccionalmente con instancia cloud mediante gRPC comprimido~\cite{gartnerMagicQuadrantIoT2024}. Ventajas incluyen dashboards locales sin internet, offline buffering automático (>100K mensajes), y rule engine CEP completo (filtros, transformaciones, alarmas).

\textbf{AWS IoT Greengrass:} Framework de AWS para ejecutar funciones Lambda en edge devices, con sincronización automática hacia AWS IoT Core. Requiere Amazon Linux 2 y conectividad periódica para deployment de funciones.

\textbf{Azure IoT Edge:} Containers Docker orquestados con módulos de runtime Azure, permitiendo ejecutar Azure Stream Analytics y ML models (ONNX) localmente.
...
\begin{itemize}
\item \textbf{CAPEX hardware:} \$1,200-2,000 USD/unidad × 100 = \$120,000-200,000 USD
\item \textbf{CAPEX software:} Windows 10 IoT Enterprise licencias \$100/dispositivo × 100 = \$10,000 USD (alternativa Ubuntu Snappy sin costo)
\item \textbf{OPEX conectividad:} LTE M2M \$20/mes × 100 × 60 meses = \$120,000 USD
\item \textbf{OPEX gestión:} Dell IoT Gateway Management Console \$5/device/mes × 100 × 60 = \$30,000 USD
...
\midrule
\textbf{TCO 5 años (100 gateways)} & \$798,000 (\$7,980/gw) & \$480,000 (\$4,800/gw) & \$136,080 (\$1,361/gw) & \textbf{\$12,400 (\$124/gw)} \\
\addlinespace[0.1cm]
\textbf{CAPEX hardware} & \$2,500-4,000 & \$1,200-2,000 & \$75 (RPi 4) & \textbf{\$55-75 (RPi 4)} \\
\addlinespace[0.1cm]
\textbf{OPEX recurrente anual} & \$55,600 (licencias + LTE + soporte) & \$30,000 (LTE + gestión) & \$6,096/año (Greengrass + IoT Core + LTE) & \textbf{\$0/año (sin licencias, HaLow sin OPEX)} \\
\addlinespace[0.1cm]
\textbf{Cumplimiento IEEE 2030.5 nativo} & No (requiere dev custom) & No (requiere plugin \$800) & No (requiere Lambda custom) & \textbf{Sí (adapter ThingsBoard-SEP2)} \\
\addlinespace[0.1cm]
\textbf{Soporte LwM2M gestión} & No & No & No & \textbf{Sí (Leshan + Eclipse Wakaama)} \\
\addlinespace[0.1cm]
\textbf{Dashboard local offline} & Sí (IOS CLI) & Limitado (Gateway Manager UI básica) & No (solo AWS Console cloud) & \textbf{Sí (ThingsBoard UI completa)} \\
\addlinespace[0.1cm]
\textbf{Operación offline indefinida} & Sí & Sí & \textbf{No (max 7 días)} & \textbf{Sí (sincronización diferida)} \\
\addlinespace[0.1cm]
\textbf{Vendor lock-in} & Alto (ecosistema Cisco propietario) & Medio (Windows IoT licencias) & Alto (AWS services tightly-coupled) & \textbf{Nulo (open-source, APIs estándar)} \\
\addlinespace[0.1cm]
\textbf{Portabilidad multi-cloud} & Baja & Media & Baja (reescritura Lambda) & \textbf{Alta (MQTT/REST estándar)} \\
\addlinespace[0.1cm]
\textbf{Performance edge analytics} & Alta (P1020 dual-core) & Baja (Atom single-core 1.46 GHz) & Depende hardware host & \textbf{Media-Alta (RPi 4 quad-core 1.8 GHz)} \\
\addlinespace[0.1cm]
\textbf{Robustez térmica} & $-40^\circ$C a $+70^\circ$C (IP30) & $-40^\circ$C a $+70^\circ$C (fanless) & Depende hardware host & $-40^\circ$C a $+85^\circ$C (\textbf{RPi CM4 + IP67}) \\
\addlinespace[0.1cm]
\textbf{Actualizaciones OTA} & Sí (Smart Licensing) & Sí (Dell Gateway Manager) & Sí (AWS deployments) & \textbf{Sí (Ansible + script bash)} \\
\addlinespace[0.1cm]
\textbf{Soporte técnico} & SmartNet 24×7 (\$55K/año) & ProSupport Plus (\$15K/año) & AWS Support Plan (\$15K/año mínimo) & \textbf{Comunidad + documentación} \\
\bottomrule
\end{tabular}
\end{table}
